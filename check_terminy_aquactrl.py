...
a jednou (ne opakovaně) o tom pošle push zadavateli, řešiteli a TŘ –
...
def main():
    ...
    for key, u in snap.items():
        ...
        if u.get('poterminu_upozorneno'):
            continue
        # Politika (shodná s appkou i Cloud Function): při nesplnění do termínu
        # se upozorní vždy zadavatel + řešitel + technický ředitel (TŘ).
        # Appka to od v82 ukládá do pole "upozornit"; tady to pro jistotu
        # doplníme i pro starší úkoly, aby platila stejná pravidla.
        targets = list(u.get('upozornit') or [])
        for extra in (u.get('zadal'), u.get('resitel'), 'TŘ'):
